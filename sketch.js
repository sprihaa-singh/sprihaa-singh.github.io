...
function setup() {
  ...
  //Initialize toppings by curr time
...
function draw() {
  ...
  if (m !== lastMinute) {
    ...
    // Reset toppings at new hr
    if (m === 0) {
      toppingPositions = [];
      hourStartMinute = m;
    }

    // Add a new pepperoni for the current minute
    if (toppingPositions.length < 60) {
      addTopping(h); // Add a new topping based on current hour
    }
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (i === h - 1) {
      continue; //skip slice for curr hr
    }
...
//Add new topping @ random position w/o overlap with other mins or in missing slice
